...
var gua64 = {
    "gua": [{
        ...
        "gua-name": "坤",
        "gua-xiang": "000000",
        "gua-detail": "元亨，利牝马之贞。君子有攸往，先迷后得主，利西南得朋，东北丧朋。安贞，吉。",
        "yao-detail": [
        "履霜，坚冰至。",
        "直，方，大，不习无不利。",
        "含章可贞。或从王事，无成有终。",
        ...
        "gua-name": "小过",
        "gua-xiang": "001100",
        "gua-detail": "亨，利贞，可小事，不可大事。飞鸟遗之音，不宜上，宜下，大吉。",
        "yao-detail": [
        "飞鸟以凶。",
        "过其祖，遇其妣；不及其君，遇其臣；无咎。",
        "弗过防之，从或戕之，凶。",
...
function JingGua(guaXiang){
    var gua = gua8.gua;
    function getJinGuaName(guaXiang){
        for(var i in gua){
            if(gua[i]['gua-xiang'] == guaXiang){
                return gua[i]['gua-name'];
            }
        }
    }
    function getJinGuaSex(guaXiang){
        for(var i in gua){
            if(gua[i]['gua-xiang'] == guaXiang){
                return gua[i]['gua-sex'];
            }
        }
    }
    function getJinGuaMean(guaXiang){
        for(var i in gua){
            if(gua[i]['gua-xiang'] == guaXiang){
                return gua[i]['gua-mean'];
            }
        }
    }
    function getJinGuaWuXing(guaXiang){
        for(var i in gua){
            if(gua[i]['gua-xiang'] == guaXiang){
                return gua[i]['wuxing'];
            }
    ...
    this.xiang = guaXiang;
    this.name = getJinGuaName(this.xiang);
    this.sex = getJinGuaSex(this.xiang);
    this.wuXing = getJinGuaWuXing(this.xiang);
    this.mean = getJinGuaMean(this.xiang);
}
function Gua(guaXiang){
    var gua = gua64.gua;
    function getGuaName(guaXiang){//卦名
        for(var i in gua){
            if(gua[i]['gua-xiang'] == guaXiang){
                return gua[i]['gua-name'];
            }
    ...
    function getZhen(guaXiang){
        return guaXiang.substr(0,3);
    }
    function getHui(guaXiang){
        return guaXiang.substr(3,3);
    }
    function getGuaCi(guaXiang){
        for(var i in gua){
            if(gua[i]['gua-xiang'] == guaXiang){
                return gua[i]['gua-detail'];
            }
        }
    }
    function getYaoCi(guaXiang){
        for(var i in gua){
            if(gua[i]['gua-xiang'] == guaXiang){
                return gua[i]['yao-detail'];
            }
        }
    }
    function getGuaMean(zhen, hui, guaName) {
        jgz = new JingGua(zhen);
        jgh = new JingGua(hui);
        var first = jgh.mean;
        var last = jgz.mean;
        if(first === last){
            return guaName + '为' + first;
        }else{
            return first + last + guaName;
        }
    }
    function getYaoOrderList(xiang){
        var idStrList = ['初', '二', '三', '四', '五', '上'];
        var yinYang = guaXiang[order] === '1' ? '九' : '六';
        var list = new Array(6);
        for(var order=0;order<list.length;order++){
            if (order === 0 || order === 5) {
                var yaoDetail = idStrList[order] + yinYang;
            } else {
                var yaoDetail = yinYang + idStrList[order];
            }
            list[order] = yaoDetail;
        }
        return list;
    }
    this.xiang = guaXiang;
    this.name = getGuaName(this.xiang);
    this.zhen = getZhen(this.xiang);//贞
    this.hui = getHui(this.xiang);//悔
    this.guaMean = getGuaMean(this.zhen, this.hui, this.name);
    this.guaCi = getGuaCi(this.xiang);
    this.YaoCi = getYaoCi(this.xiang);
    this.yaoOrder = getYaoOrderList(this.xiang);
}